# 🔍 Finsearcher - AI 투자 어드바이저

Finsearcher는 **Streamlit** 기반의 AI 투자 어드바이저 챗봇 애플리케이션입니다. LangChain, LangGraph를 활용하여 실시간 주가 분석, 뉴스 감성 분석, 포트폴리오 관리 등 종합적인 투자 분석 서비스를 제공합니다.
...
이 프로젝트는 OpenAI API를 사용합니다. 아래 단계를 따라 API 키를 발급받으세요.

#### 📝 API 키 발급 방법

1. **OpenAI 계정 생성**
   - [OpenAI 홈페이지](https://platform.openai.com/) 접속
   - 우측 상단 `Sign Up` 클릭하여 회원가입
   - 이메일 인증 완료

2. **API 키 생성**
   - 로그인 후 [API Keys 페이지](https://platform.openai.com/api-keys) 이동
   - `+ Create new secret key` 버튼 클릭
   - 키 이름 입력 (예: "Finsearcher")
   - 생성된 키를 **즉시 복사** (다시 볼 수 없음!)

3. **결제 수단 등록 (필수)**
   - [Billing 페이지](https://platform.openai.com/account/billing) 이동
   - `Add payment method`로 신용카드 등록
   - 초기 크레딧 $5~$10 충전 권장

4. **사용량 제한 설정 (권장)**
   - [Usage limits 페이지](https://platform.openai.com/account/limits) 이동
   - 월별 사용 한도 설정 (예: $10)
...
|------|---------------|---------------|------|
| GPT-5 nano | $0.05 | $0.4 | 일반 대화, 분석 |
| GPT-5 mini | $0.25 | $2 | 분석 |
| GPT-4o mini | $0.15 | $0.6 | (과거)일반 대화 및 분석 |

> 💰 **예상 비용**: 일반적인 사용 시 월 $1~$5 수준
...
> ⚠️ **주의**: `.env` 파일은 절대 GitHub에 업로드하지 마세요! (`.gitignore`에 추가)
